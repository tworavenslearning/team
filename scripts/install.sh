#!/usr/bin/env bash
#
# install.sh — Install Enablement Agents into your local agentic tool(s).
#
...
# is missing or stale. Claude Code copies directly from agent dirs.
#
# Usage:
#   ./scripts/install.sh [--tool <name>] [--interactive] [--no-interactive] [--help]
#
# Tools:
#   claude-code  — Copy agents to ~/.claude/agents/ (from agent dirs directly)
#   antigravity  — Copy skills to ~/.gemini/antigravity/skills/
#   gemini-cli   — Install extension to ~/.gemini/extensions/team/
...
#   --no-interactive  Skip interactive selector, install all detected tools (CI)
#   --help            Show this help
#
# Platform support:
...
AGENT_DIRS=(content-intelligence content-architecture feedback-insights standards-governance metrics-outcomes orchestration)
...
# ---------------------------------------------------------------------------
# Preflight — require integrations/ only for tools that need it
# ---------------------------------------------------------------------------
check_integrations() {
  local needs=false
  for t in "$@"; do
...
interactive_select() {
  ...
  while true; do
    printf "\n"
    box_top
    box_row "${C_BOLD}  Enablement Agents — Tool Installer${C_RESET}"
    box_bot
    printf "\n"
...
install_gemini_cli() {
  ...
  local dest="${HOME}/.gemini/extensions/team"
  local count=0
...
main() {
  ...
  if $use_interactive; then
    ...
    header "Enablement Agents — Scanning for installed tools..."
    printf "\n"
    local t
    for t in "${ALL_TOOLS[@]}"; do
      if is_detected "$t" 2>/dev/null; then
  ...
  check_integrations "${SELECTED_TOOLS[@]}"

  printf "\n"
  header "Enablement Agents — Installing agents"
  printf "  Repo:       %s\n" "$REPO_ROOT"
  printf "  Installing: %s\n" "${SELECTED_TOOLS[*]}"
  printf "\n"
